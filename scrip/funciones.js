...
export function createEventCard(event, tarjetaId) {
      ...
      <div class="tarjetas">
        ...
        <div class="card-body">
          ...
          <div class="d-flex justify-content-between align-items-end mt-auto">
            ...
            <a href="Details.html?id=${event._id}" class="btn btn-primary m-2">Go somewhere</a>
          </div>
        </div>
      </div>
    `;
    const btnDetails = cardDiv.querySelector('.btn-primary');
    btnDetails.href = `Details.html?id=${event._id}`;
...
export function updateContent(events) {
...
export function createCarouselContainers(events, itemsPerContainer) {
    ...
    for (let i = 0; i < events.length; i += itemsPerContainer) {
      const carouselItem = document.createElement("div");
      carouselItem.classList.add(i === 0 ? "carousel-item active" : "carousel-item");
...
export function handleSearchEvent(e) {
    const searchText = e.target.value.toLowerCase();
    const checkedCategories = getCheckedCategories();
    applyFilters(data.events, searchText, checkedCategories);
}   
export function applyFilters(events, searchText, checkedCategories) {
    ...
    createCarouselContainers(filteredEvents, 4);
}
...
export function setCheckboxListeners() {
    const checkboxes = document.querySelectorAll('input[type="checkbox"]');
    checkboxes.forEach(checkbox => {
        checkbox.addEventListener('change', handleCheckboxChange);
    });
}
export function filterComingEvents(events) {
