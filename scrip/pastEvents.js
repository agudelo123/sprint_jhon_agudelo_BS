...
function updateContent(events) {
  ...
  if (events.length === 0) {
    ...
    noEventsMessage.textContent = "No past events found that match the criteria.";
...
// Nos aseguramos de que también se llame a updateContent después de una búsqueda o una operación de filtrado por categoría.
...
function applyFilters(filteredEvents = filterPastEvents(data.events)) {
